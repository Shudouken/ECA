...
$\Rightarrow$ Compute fingerprints of all generated polyominos: $\mathcal{O}(k \cdot n)$

$\Rightarrow$ Sort all fingerprints: $\mathcal{O}(k \cdot log(k) \cdot n)$, duplicates are neighboured in the sorting and removing takes $\mathcal{O}(k \cdot n)$
$\Rightarrow$ the total runtime is $\mathcal{O}(k \cdot log(k) \cdot n)$
